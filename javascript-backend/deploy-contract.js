...
const deployContractWeb3Hard = async () => {
  const rpcURL = INFURA_ROPSTEN_ENDPOINT;
  const web3 = new Web3(rpcURL);
  const networkId = await web3.eth.net.getId();

  const gas = await web3.eth.estimateGas({ data: data });
  const gasPrice = await web3.eth.getGasPrice();
  const nonce = await web3.eth.getTransactionCount(ADDRESS_1);
...
const deployContractWeb3Normal = async () => {
  const rpcURL = INFURA_ROPSTEN_ENDPOINT;
  const web3 = new Web3(rpcURL);
  const networkId = await web3.eth.net.getId();
...
const deployContractWeb3Easy = async () => {
  const rpcURL = INFURA_ROPSTEN_ENDPOINT;
  const web3 = new Web3(rpcURL);
  const networkId = await web3.eth.net.getId();
  web3.eth.accounts.wallet.add(PRIVATE_KEY_STR_1);
  ...
  contract
    .deploy({ data: data, arguments: ["hello world"] })
    .send({ from: ADDRESS_1, gasLimit: gas })
    .then((txHash) => {
      console.log("[web3]", "txHash:", txHash);
    });
};

const deployContractEthersEasy = async () => {
...
const deployContractEthersNormal = async () => {
  const provider = new ethers.providers.InfuraProvider(
    "ropsten",
    INFURA_PROJECT_ID
  );
  const signer = new ethers.Wallet(PRIVATE_KEY_STR_1, provider);
  const nonce = await provider.getTransactionCount(ADDRESS_1);
  const gas = await provider.estimateGas({ data: data });
  ...
  signer
    ...
    .then((transaction) => {
      console.log("[ethers]", "txHash", transaction);
    });
};

// deployContractWeb3Tx();
// deployContractWeb3Hard();
// deployContractWeb3Normal();
// deployContractWeb3Easy();

// deployContractEthersEasy();
deployContractEthersNormal();
